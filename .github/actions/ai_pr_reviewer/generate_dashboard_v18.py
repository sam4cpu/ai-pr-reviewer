...
# Fix: set subplot types for compatibility with Indicator
fig = make_subplots(
    rows=2, cols=2,
    subplot_titles=("Predictive Insights","Reward Matrix","Network Weights","Reviewer Confidence"),
    specs=[[{"type": "xy"}, {"type": "domain"}],   # Predictive = xy, Reward = domain (gauge)
           [{"type": "xy"}, {"type": "domain"}]]   # Network = xy, Confidence = domain
)
...
if network:
    ...
    fig.add_trace(go.Bar(
        x=list(w.keys()),
        y=[float(w[k]) if isinstance(w[k],(int,float)) else 0 for k in w.keys()],
